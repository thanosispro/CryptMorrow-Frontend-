            ...
            >
              ...
              <div className="flex-grow p-0 overflow-auto relative z-10 custom-scrollbar scroll-smooth bg-slate-50/30">
                <table className="w-full text-left border-collapse min-w-[900px]">
                  <thead className="text-[10px] text-text-tertiary font-black uppercase bg-white/80 backdrop-blur-xl border-b border-border-subtle sticky top-0 z-20">
                    <tr>
                      <th className="px-10 py-6 tracking-[0.3em]">Date & Time</th>
                      <th className="px-6 py-6 tracking-[0.3em]">Price Info</th>
                      <th className="px-6 py-6 tracking-[0.3em] text-secondary">Indicators</th>
                      <th className="px-6 py-6 tracking-[0.3em] text-primary">Patterns</th>
                      <th className="px-10 py-6 tracking-[0.3em] text-right">Volume</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border-subtle font-medium">
                    <AnimatePresence>
                      {data ? data.map((d, i) => (
                        ...
                        >
                          <td className="px-10 py-8 text-text-tertiary text-[11px] font-mono whitespace-nowrap uppercase tracking-tighter">
                            {new Date(d.time * 1000).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }).replace(',', '')}
                          </td>
                          <td className="px-6 py-8">
                            <div className="flex items-center gap-4">
                              <div className="text-lg font-black text-text-primary tracking-tighter">${d.close?.toLocaleString()}</div>
                              <div className={`px-2.5 py-1 rounded-xl text-[10px] font-black border flex items-center gap-1 ${d.close >= d.open ? 'bg-success/5 text-success border-success/10' : 'bg-error/5 text-error border-error/10'}`}>
                                {d.close >= d.open ? <ArrowRight className="w-3 h-3 -rotate-45" /> : <ArrowRight className="w-3 h-3 rotate-45" />} {Math.abs(((d.close - d.open) / d.open) * 100).toFixed(2)}%
                              </div>
                            </div>
                            <div className="flex gap-4 mt-2 font-mono text-[10px] text-text-tertiary uppercase font-bold tracking-tighter">
                              <span className="bg-slate-100/50 px-2.5 py-1 rounded-lg">O: {d.open?.toFixed(1)}</span>
                              <span className="bg-slate-100/50 px-2.5 py-1 rounded-lg">H: {d.high?.toFixed(1)}</span>
                              <span className="bg-slate-100/50 px-2.5 py-1 rounded-lg">L: {d.low?.toFixed(1)}</span>
                            </div>
                          </td>
                          <td className="px-6 py-8">
                            <div className="space-y-4">
                              <div className="flex items-center gap-4">
                                ...
                                <span className={`text-[11px] font-black font-mono w-10 ${d.rsi > 70 ? 'text-error' : d.rsi < 30 ? 'text-success' : 'text-text-primary'}`}>{d.rsi?.toFixed(1)}</span>
                              </div>
                              <div className="flex items-center gap-2">
                                <span className="text-[10px] font-black text-text-tertiary uppercase tracking-widest">MACD:</span>
                                <span className={`text-[11px] font-black font-mono ${d.macd > d.macd_signal ? 'text-success' : 'text-error'}`}>{d.macd?.toFixed(4)}</span>
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-8">
                            <div className="flex flex-wrap gap-2.5">
                              {d.pattern_doji && (
                                <span className="bg-primary/5 text-primary px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] border border-primary/20 shadow-sm animate-float">Doji_Candle</span>
                              )}
                              {d.pattern_hammer && (
                                <span className="bg-secondary/5 text-secondary px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] border border-secondary/20 shadow-sm animate-float">Hammer_Candle</span>
                              )}
                              {!d.pattern_doji && !d.pattern_hammer && (
                                <div className="flex items-center gap-2 opacity-30">
                                  <div className="w-2 h-2 rounded-full bg-slate-300" />
                                  <span className="text-text-tertiary text-[10px] font-black uppercase tracking-widest italic">No Pattern</span>
                                </div>
                              )}
                            </div>
                          </td>
                          <td className="px-10 py-8 text-right">
                            <div className="text-text-primary text-sm font-black tracking-tighter">${(d.volumeto / 1000000).toFixed(2)}M</div>
                            <div className="text-[10px] text-text-tertiary mt-1 font-black uppercase tracking-widest opacity-60">Market Volume</div>
                          </td>
                        </motion.tr>
                      )) : (
                        <tr>
                          <td colSpan="5" className="py-64">
                            <div className="flex flex-col items-center justify-center text-center px-10">
                              <h4 className="text-2xl font-display font-extrabold text-text-primary tracking-tight uppercase">No Data Loaded</h4>
                              <p className="text-text-secondary text-lg mt-4 max-w-sm font-medium leading-[1.6]">Select a coin and timeframe above to load historical market data and start your analysis.</p>
                            </div>
                          </td>
                        </tr>
                      )}
